...
const getNavMenu = ()=>{
    return Axios.get("/blog/nav_menu.json")
}

const getSiteConfig = ()=> {
    return Axios.get("/blog/site_config.json")
}

export const getArticleList = (dispatch,action,) => {
    Axios.get("/blog/article/list.json")
...
export const getArticle = (dispatch,action,id) => {
    Axios.get("/blog/article/posts.json")
    .then(function(article_info){
        if(article_info.data.content_path!==""){
            var content_path = "/blog/posts/"+id+".md"
